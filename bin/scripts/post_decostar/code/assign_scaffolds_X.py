...
for id_gene_family in reconciled_gene_trees.keys():
  ...
  for l in leaves:
    ...
    if event != "Loss":
      ...
      name_leaf = script_tree.getName(tree,l).split("|")[0].split("@")[1]
      #print name_leaf
  ...
  for n in nodes:
    if script_tree.isLeaf(tree,n):
      ...
      species_name = name.split("|")[0].split("@")[0]
    else:
    ...
    if event == "Extant" or event == "Spe":
        ...
        for l in gambia:
            ...
            if script_tree.isLeaf(tree,common_ancestor) or (script_tree.getBootstrap(tree,common_ancestor).split("|")[1] == "Spe"):
              orthologs[name].append(script_tree.getName(tree,l).split("|")[0].split("@")[1])
...
for c in chr_names:
    assignment_file.write(c+" "+str(len(chromosomes[c]))+" "+str(distrib[c]["X"])+" "+
                          str(distrib[c]["2L"])+" "+str(distrib[c]["2R"])+" "+
                          str(distrib[c]["3L"])+" "+str(distrib[c]["3R"])+"\n")
